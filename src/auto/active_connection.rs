...
impl<O: IsA<ActiveConnection> + IsA<glib::object::Object>> ActiveConnectionExt for O {
    fn get_connection(&self) -> Option<RemoteConnection> {
        unsafe {
            from_glib_none(ffi::nm_active_connection_get_connection(self.to_glib_none().0))
        }
    }
    ...
    fn get_master(&self) -> Option<Device> {
        unsafe {
            from_glib_none(ffi::nm_active_connection_get_master(self.to_glib_none().0))
        }
    }
    ...
    fn connect_property_state_flags_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe {
            let f: Box_<Box_<Fn(&Self) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "notify::state-flags",
                transmute(notify_state_flags_trampoline::<Self> as usize), Box_::into_raw(f) as *mut _)
        }
    }
